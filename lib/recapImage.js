...
// niente emoji: il rendering dei glifi emoji su <canvas> è troppo
// incostante da un browser/OS all'altro (spesso escono contorni grigi
// vuoti). Un badge con le iniziali, come le carte giocatore, è
// coerente con lo stile dell'app e non dipende da nessun font esterno.
function iniziali(nome) {
  return (nome || "?").split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();
}

function rigaPremio(ctx, y, { accento, etichetta, nome, valore }) {
  ...
  ctx.beginPath();
  ctx.arc(158, y - 8, 44, 0, Math.PI * 2);
...
export async function generaImmagineRecap(datiPartita) {
  ...
  const sottotitolo = datiPartita.legaNome ? `${datiPartita.legaNome} · ${datiPartita.dataTesto}` : datiPartita.dataTesto;
  scriviCentrato(ctx, sottotitolo, W / 2, 158, { font: "700 24px Archivo, sans-serif", colore: MUTO });
  ...
  let y = 660;
  if (datiPartita.mvp) {
    rigaPremio(ctx, y, { accento: ORO, etichetta: "MVP della serata", nome: datiPartita.mvp.nome, valore: datiPartita.mvp.voto?.toFixed(2) });
    y += 138;
  }
  if (datiPartita.scarso) {
    rigaPremio(ctx, y, { accento: "#E05C4B", etichetta: "Bidone della serata", nome: datiPartita.scarso.nome, valore: datiPartita.scarso.voto?.toFixed(2) });
    y += 138;
  }
  if (datiPartita.marcatore) {
    rigaPremio(ctx, y, { accento: "#5CBF7A", etichetta: "Marcatore di serata", nome: datiPartita.marcatore.nome, valore: `${datiPartita.marcatore.gol}` });
  }
