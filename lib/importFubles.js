...
export function parseImportRapido(dati, { legaId, partiteEsistenti, giocatoriEsistenti }) {
  ...
  const esistente = (partiteEsistenti || []).find((p) => p.match_id === String(dati.match_id));
  let partitaEsistenteId = null;
  if (esistente) {
    if (!esistente.stato_live) {
      errori.push(`Questa partita (match_id ${dati.match_id}) risulta già importata in questa lega.`);
      return { errori, avvisi, partita: null, giocatoriNuovi: [], prestazioni: [], voti: [] };
    }
    // era solo la formazione pre-partita del live match: questo è il
    // risultato finale, si aggiorna la stessa riga invece di bloccare
    partitaEsistenteId = esistente.id;
  }
  ...
  if (partitaEsistenteId) partita.stato_live = "conclusa";
  ...
  return { errori: [], avvisi, partita, giocatoriNuovi, prestazioni, voti, partitaEsistenteId };
}
