...
function parseTabella(testo) {
  ...
  const dati = righe.slice(1).map((riga) => {
    ...
    const obj = {};
    intestazioni.forEach((h, i) => { obj[h] = (celle[i] ?? "").trim(); });
    return obj;
  });
...
export function parseImportFubles({ partiteText, prestazioniText, votiText, legaId }) {
  ...
  if (errori.length) return { errori, partite: [], giocatoriNuovi: [], prestazioni: [], voti: [] };
  ...
  const voti = votiRaw.righe
    ...
    .filter((v) => v.valutato_fubles_user_id && v.votante_fubles_user_id && v.voto != null);
