...
export function applicaNomiRegistrati(giocatori, mappa) {
  if (!mappa || !Object.keys(mappa).length) return giocatori;
  return giocatori.map((g) => (mappa[g.id] ? { ...g, nome: mappa[g.id] } : g));
}

/* ---------- assemblaggio dati dal DB ---------- */
export function assemble(partite, giocatori, prestazioni, votiRaw) {
  const P = {};
